...
<script lang="ts">
  ...
  import { retrieveBookBorrowingApi } from '/@/api/library/BookBorrowingApi';

  const record = ref({});

  export default defineComponent({
    name: 'BookBorrowingDetailDrawer',
    components: { BasicDrawer, Description },
    emits: ['success', 'register'],
    setup(_) {
      const [registerDrawer] = useDrawerInner(async (data) => {
        record.value = await retrieveBookBorrowingApi(data.record.id);
      });
