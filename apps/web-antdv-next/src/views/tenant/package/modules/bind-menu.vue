...
const getTitle = computed(() =>
  $t('tenant.package.grantTitle', [packageName.value]),
);
...
<template>
  <Drawer
    ...
    :confirm-text="$t('tenant.package.savePermissions')"
  >
    <Spin :spinning="loading" :classes="{ root: 'w-full' }">
      <!-- 工具栏：关联/独立、折叠、刷新 -->
      <div class="mb-2 flex items-center justify-end gap-2 pt-2">
        <Segmented
          v-model:value="linkMode"
          :options="[
            { label: $t('tenant.package.nodeLinked'), value: 'linked' },
            {
              label: $t('tenant.package.nodeIndependent'),
              value: 'independent',
            },
          ]"
          size="small"
        ...
        <Button size="small" @click="onToggleExpand">
          ...
          {{
            collapsed
              ? $t('tenant.package.expand')
              : $t('tenant.package.collapse')
          }}
        </Button>
      ...
      >
        ...
        >
          ...
          />
          <span>{{ $t('tenant.package.menuCol') }}</span>
          <span>{{ $t('tenant.package.permCol') }}</span>
        </div>
